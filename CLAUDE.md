...
- **A migration's `_journal.json` `when` must be greater than the entry before it.**
  A test enforces this (`migration-journal.test.ts`) — if it fails, raise the new
  migration's `when`; don't lower anything else.
  *Why it matters:* `migrate()` reads the highest `created_at` in the database **once,
  before applying anything**, then skips any migration whose `when` is below it. On an
  empty database nothing is skipped, so **no test suite here can catch this** — they all
  build from scratch. On an existing database, i.e. production, a low-stamped migration is
  skipped **silently**: no error, no DDL, `migrate()` reports success. Demonstrated: with
  `0021` stamped a day in the future, a two-stage deploy applied 26 of 32 migrations and
  never created the `categories` table, reporting OK both times. `0021` has been restamped
  and the journal is monotonic.
  *If you ever have to restamp again, lower the offender rather than raising what follows
  it* — raising re-runs already-applied migrations on live databases.
