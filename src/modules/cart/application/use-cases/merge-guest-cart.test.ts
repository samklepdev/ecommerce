...
describe('MergeGuestCart', () => {
  it('merges guest lines into the user cart and deletes the guest cart', async () => {
    ...
    const guestCart = makeCart({ type: 'guest', sessionId: 's1' }, [
      CartLine.create({ variantId: variantA, sku: 'A', quantity: 1, unitPrice: Money.of(1000, 'USD') }),
    ]);
  ...
  it('sums quantities on a variant collision between guest and user carts', async () => {
    const variantA = randomUUID();
    const guestCart = makeCart({ type: 'guest', sessionId: 's1' }, [
      CartLine.create({ variantId: variantA, sku: 'A', quantity: 2, unitPrice: Money.of(1000, 'USD') }),
    ]);
    const userCart = makeCart({ type: 'user', userId: 'user-1' }, [
      CartLine.create({ variantId: variantA, sku: 'A', quantity: 3, unitPrice: Money.of(1000, 'USD') }),
    ]);
  ...
  it('returns the existing user cart unchanged when there is no guest cart', async () => {
    const userCart = makeCart({ type: 'user', userId: 'user-1' }, [
      CartLine.create({ variantId: randomUUID(), sku: 'X', quantity: 1, unitPrice: Money.of(500, 'USD') }),
    ]);
